...
public sealed class MainViewModel : ViewModelBase
{
    ...
    private async Task OnPlayAsync()
    {
        ...
        IsPlaying = true;
        StatusMessage = $"Воспроизведение «{script.Name}»… (F5 ещё раз или Стоп — прервать)";
        if (_settingsService.Current.Playback.EnableStallWatchdog)
            StartStallWatchdog();

        try
        {
            if (ExecutionGraphPersistence.TryLoad(script, out var executionGraph) && executionGraph is not null)
            ...
            StatusMessage = $"Воспроизведение «{script.Name}» завершено.";
        }
